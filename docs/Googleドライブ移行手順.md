...
> 当初ここには「Googleドライブ配下には絶対に向けないこと」と書いていた。理由は
> ①コピー途中の状態が同期される ②ストリーミングだと実体がローカルにない、の2点。
> しかし**バックアップが社内(C:とNAS)にしか無い状態はランサムウェア・火災で全滅する**ため、
> 社外に1本置く必要があると判断し、次の対応をしたうえでGoogle共有ドライブを保存先に追加した。
>
> - ①への対応: `lib/db-backup.js` が**一時ファイル名で書いてから rename** する方式に変更済み。
>   同期側が拾うのは完成後のファイルだけになり、書きかけが正規名で同期されることはない
> - ②は**読み出し(DESIGNデータを開く)の話**であり、バックアップの書き込みには影響しない。
>   復元時はGoogleドライブからダウンロードして使う
>
> 現在の設定は README「バックアップはどこに何本あるか」を正とする。
