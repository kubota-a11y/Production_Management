...
(function () {
  ...
  function calcRowBase(row, qty, minQty, opt) {
    ...
    if (row.method === 'marking') {
      ...
      // マーキングにもミニマム手数料を掛けない(2026-09-02 社長確定。ミニマムが残るのはシルクのみ)
      return { label: `マーキング ${m.name}(${m.size})`, short: 'マーキング', cust: `マーキング ${m.name}`, unit: mulDtf(m.p), initial: 0, note: window.QS_MARKING_NOTE, minFeeExempt: true };
    }
    ...
    if (row.method === 'rubber') {
      ...
      /* ラバー転写にはミニマム手数料を掛けない(2026-09-02 社長確定。加工料金表の
         印字どおりで、DTF(2026-09-01)と同じ整理。「1枚〜」の段が既に小口の割高
         単価を織り込んでいるため。特殊シート等の割増・特急はそのまま掛かる */
      return { label: `ラバー転写 ${row.size}`, short: 'ラバー転写', cust: `ラバー転写プリント(${row.size}以内)`, unit: mulDtf(u), initial: 0, note: '', minFeeExempt: true };
    }
    ...
    box.innerHTML = `
      ...
      ${r.mode === 'yagi'
        ...
        : r.mode === 'kratvs'
          ...
          : '<p class="qs-note">単価はすべて税抜の概算です(消費税は小計に対して加算)。10枚未満のミニマム手数料(加工代5割増)が掛かるのはシルクスクリーンのみです(他の加工は対象外・自動判定)。</p>'}`;
  }
