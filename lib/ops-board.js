...
function registerOpsBoardRoutes(app, db) {
  ...
  // ---- ボードから外す ----
  // is_design_ops を0に戻すだけ。案件そのものは消さない(案件の削除は一覧ビューから行う)。
  // 導入時のマイグレーションで自動的にフラグが立った案件を、山本さんが片付けるために使う
  app.patch('/api/ops/cases/:id/remove', (req, res) => {
    try {
      const caseId = parseInt(req.params.id, 10);
      const project = db.prepare('SELECT id, project_name FROM projects WHERE id = ?').get(caseId);
      if (!project) return res.status(404).json({ ok: false, error: '案件が見つかりません' });

      db.prepare('UPDATE projects SET is_design_ops = 0, updated_at = ? WHERE id = ?')
        .run(new Date().toISOString(), caseId);
      console.log(`[デザイン案件全般] 案件#${caseId} をボードから外しました(案件は残っています)`);
      res.json({ ok: true, project_name: project.project_name });
    } catch (err) {
      console.error('[デザイン案件全般] ボードから外す処理でエラー:', err.message);
      res.status(500).json({ ok: false, error: '更新に失敗しました' });
    }
  });
