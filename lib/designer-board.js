...
// 稼働申告で時間数だけ指定された場合の開始時刻。社内ボードの表示と整合させるための便宜値
...
function getDayInfo(db, employeeId, dateISO) {
  ...
  if (override) {
    if (override.is_day_off) return { hours: 0, source: 'override', is_day_off: true };
    return {
  ...
  if (!def || !def.is_working) return { hours: 0, source: 'default', is_day_off: true };
  return {
...
function registerDesignerBoardRoutes(app, db, deps = {}) {
  ...
  app.get('/api/designer/:token/board', async (req, res) => {
    try {
      ...
      const sheet_todos = allSheetTodos ? extractOpenTodosFor(allSheetTodos, link.employee_name) : null;
  ...
  app.put('/api/designer/:token/items/:id', (req, res) => {
    try {
      ...
      if (status !== item.status) syncCaseStatus(item.case_id);
  ...
  app.post('/api/designer/:token/availability', (req, res) => {
    try {
      ...
      if (mode === 'clear') {
        ...
        // 時間数のみの申告のため、便宜的に 09:00 開始で開始/終了時刻に変換して保存する
        // (社内ボードの表示・空き時間計算は開始/終了時刻ベースのため)
        ...
        if (existing) {
          db.prepare(`UPDATE schedule_overrides SET is_day_off=0, start_time=?, end_time=?, break_minutes=0 WHERE id=?`)
            .run(AVAILABILITY_START, end_time, existing.id);
        } else {
          db.prepare(`
            INSERT INTO schedule_overrides (employee_id, work_date, start_time, end_time, break_minutes, is_day_off)
            VALUES (?, ?, ?, ?, 0, 0)
          `).run(link.employee_id, work_date, AVAILABILITY_START, end_time);
        }
      } else {
