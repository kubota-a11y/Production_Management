...
function getOwnPrepItem(db, link, itemId) {
  const item = db.prepare(`
    SELECT cpi.*, pim.name as preparation_item_name, pim.code as preparation_item_code,
           ...
           p.ops_flow, p.first_draft_due, p.submission_due
...
function registerDesignerBoardRoutes(app, db, deps = {}) {
  ...
  app.get('/api/designer/:token/board', async (req, res) => {
    try {
      ...
      //
      // 2026-08-03 社長指示: デザイン案件全般ボードに載せている案件は、
      // 「入稿・製造」より前の段階のものだけをこのボードに出す。入稿が終わった案件の作業は
      // デザイン担当の手を離れているため、完了チェックが残っていてもボードから外す。
      ...
      const BEFORE_PRODUCTION_SQL = `
        (p.is_design_ops = 0
          OR p.ops_stage IN ('BRIEF', 'DESIGN', 'REVIEW')
          OR (p.ops_flow = 'SUBMIT_END' AND p.ops_stage = 'PRODUCTION'))
      `;

      // 紙媒体案件は「初稿提出」と「入稿完了」の2枚が最初から並んで見づらいので、
