...
const { buildFollowup, getLiffId } = require('./line-followup');
...
function renderPage(kind) {
  ...
  const config = {
    ...
    maxFiles: MAX_FILES,
    // LIFF ID(未設定なら空)。LINEアプリ内で開かれたときだけ、送信と同時にトークへ受付番号を自動投稿する
    liffId: getLiffId(),
    kinds: chooserList(),
...
function registerInquiryRoutes(app, db) {
  ...
  app.post('/api/inquiry/:kind', uploadMiddleware, async (req, res) => {
    ...
    try {
      ...
      // 開いた場所(liff=LINEアプリ内 / browser=それ以外)。画面側の申告なので参考情報として meta にだけ残す
      const client = payload._client === 'liff' ? 'liff' : 'browser';
      ...
      const preRaw = {
        ...
        submitted_at: now,
        images: [],
        meta: { form_version: '1.1', src, client, user_agent: s(req.headers['user-agent'], LEN.short), client_ip_hash: hashIp(ip), turnstile_verified: !ts.skipped },
      };
      ...
      try {
        ...
        // 公式LINEのトークへ「受付番号入りの本文」を送ってもらうための案内(lib/line-followup.js)。
        // 完了画面のボタン・受付控えメール・LIFFの自動投稿の3つで同じ本文を使う
        const orderer = buildOrderer(kind, values);
        const lineFollowup = buildFollowup({ receiptNo, customerName: [orderer.org_name, orderer.contact_name].filter(Boolean).join(' ') });
        ...
        if (willSendMail) {
          sendOrderConfirmation({ to: orderer.email, ...mailPayload, lineFollowup })
        ...
        return res.status(201).json({
          ...
          // text: LIFFの自動投稿に使う本文 / url: 本文入力済みでトークが開くリンク(ベーシックID未設定なら null)
          line_followup: lineFollowup,
