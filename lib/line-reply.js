...
function buildContext(lineUserId) {
  ...
  const messages = db.prepare(`
    SELECT id, direction, message_type, text_content, image_path, received_at, sent_by, sent_file_id
  ...
  `).all(...caseIds) : [];
  return { user, messages, trigger, intakes, projects, quotes, forms: formIntakes(lineUserId) };
}
...
function conversation(lineUserId, { days = 30, limit = 80 } = {}) {
  ...
  const messages = state.db.prepare(`
    SELECT id, direction, message_type, text_content, image_path, received_at, sent_by, reply_draft_id, sent_file_id
  ...
  `).all(lineUserId, lineUserId);
  return { user, messages, pendingCount, folders, intakes: formIntakes(lineUserId), filesReady: lineFiles.isReady() };
}
...
function formIntakes(lineUserId) {
  const rows = state.db.prepare(`
    ...
    WHERE (linked_line_user_id = ? OR line_user_id = ?) AND line_user_id NOT LIKE 'U%'
  ...
  properties: {
    ...
    bodies: {
      type: 'array', items: {
        ...
        required: ['sku', 'name_hint', 'qty', 'manual_unit', 'note'],
      },
    },
    rows: {
      type: 'array', items: {
...
function getQuoteContext(draftId) {
  ...
  return { draft_id: d.id, line_user_id: d.line_user_id, display_name: d.display_name, summary: d.summary, conditions: cond, freee_quotation_number: d.freee_quotation_number, freee_report_url: d.freee_report_url, quote_file_id: d.quote_file_id };
}
...
module.exports = {
  ...
  buildQuoteConditions, getQuoteContext, attachFreeeQuote,
