...
async function correctPhoto(buffer) {
  ...
  // 探索範囲。下限0.9では屋外の明るい写真が目標まで下がりきらず143で止まっていた
  // （2026-07-30に運動会の写真で発覚）。サイト側 scripts/photo.mjs と同じ値にしてある
  let lo = 0.5, hi = 2.2, best = null;
  for (let i = 0; i < 7; i++) {
    const mid = (lo + hi) / 2;
...
function registerWorksRoutes(app) {
  ...
  const upload = multer({
    ...
    limits: { fileSize: 25 * 1024 * 1024, files: MAX_PHOTOS },
