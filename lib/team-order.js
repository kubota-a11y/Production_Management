...
function todayPlusDays(days) {
  ...
  // toISOString()はUTC基準のため、JSTの深夜0〜9時に下限が1日前へずれる。ローカル日付で組み立てる
...
function registerTeamOrderRoutes(app, db) {
  ...
  app.post('/api/team-order/:token', async (req, res) => {
    try {
      ...
      // cf-connecting-ip はCloudflareエッジが実クライアントIPで上書きするため偽装不可。
      // x-forwarded-for は自己申告で偽装できるので使わない(order-intake.jsのgetClientIpと同方式)
      ...
      // 受付控えの実送信結果をレスポンスに含める(receipt_mail)。
      // 以前は画面側が「メール欄に入力があるか」だけで「お送りしました」を表示していたため、
      // SMTP未設定・送信失敗でもお客様には送信済みに見えてしまっていた
      let receiptMailSent = false;
      ...
      res.status(201).json({ ok: true, receipt_no: receiptNo, total_label: summary.totalLabel, receipt_mail: receiptMailSent });
    } catch (err) {
