...
const { notifyIntakeTask, notifyIntakeSummary } = require('./todo-notify');
...
// ```json や ``` で囲まれて返ってきた場合に備え、先頭・末尾のコードフェンスを除去する
...
// コードフェンス除去後もJSON.parseに失敗する場合の最終手段として、
// テキスト中の最初の{から最後の}までを抜き出して再度パースを試みる
...
// APIキーが未設定の環境(開発機など)ではAI抽出そのものを行わない。
// 呼び出し側は簡易的なキーワード判定へフォールバックする。
function isAiConfigured() {
  return Boolean(process.env.ANTHROPIC_API_KEY);
}

// 貼り付けたテキスト1本をLINEと同じプロンプトで構造化する。
// メール・チャットの本文取り込み(lib/manual-intake.js)から使う。
// 成功時 { parsed, raw } / APIキー未設定や失敗時は null を返す(例外は投げない)。
async function extractStructuredFromText(text) {
  ...
  try {
    ...
    console.error('[AI抽出] 貼り付けテキストのAPI呼び出し失敗:', err.message);
    return null;
  }
  ...
  try {
    return { parsed: parseAiJson(rawText), raw: response };
  } catch (err) {
    console.error(`[AI抽出] 貼り付けテキストのJSON解析失敗: ${err.message}\n応答: ${rawText}`);
    return null;
  }
}
...
async function extractForUser(db, lineUserId) {
  ...
  // 社員TODOリスト(TODO_三浦)へ通知(失敗しても抽出処理には影響しない)
  // 顧客名が取れたものだけ個別行にする。取れなかったものは1日1行のサマリに集約して
  // TODOが埋まらないようにする(詳細はHiBoardの受注候補で確認する運用)
  const taskCustomer = toBindable(parsed.customer_name);
  if (taskCustomer) {
    const taskItems = toBindable(parsed.items) || '内容は受注候補を確認';
    notifyIntakeTask(`公式LINE問い合わせ対応: ${taskCustomer} — ${taskItems}(受付 L-${intakeId})`);
  } else {
    notifyIntakeSummary(intakeId);
  }
}
