...
// 受付番号(W-/T-/P-/Q-)+ お申し込み時の電話番号下4桁 の2要素で照合する。
...
// Q- は公式LINE入口別お問い合わせ(3入口とも同じプレフィックスなので line_user_id は複数)
const PREFIX_TO_SOURCE = { W: ['WEB'], T: ['TEAM'], P: ['PARTNER'], Q: ['INQ_TEAM', 'INQ_CLASS_T', 'INQ_ORIGINAL'] };
...
function parseReceiptNo(raw) {
  const m = String(raw || '').trim().toUpperCase().match(/^([WTPQ])-?(\d{1,9})$/);
  if (!m) return null;
...
// Web注文/チーム注文は orderer.phone、取引先 加工依頼は dropoff.phone、公式LINE入口フォームは values.phone に入っている
function extractPhone(rawJson) {
  let raw;
  try { raw = JSON.parse(rawJson || '{}'); } catch { return ''; }
  const candidates = [raw?.orderer?.phone, raw?.dropoff?.phone, raw?.values?.phone];
    ...
    try {
      ...
      if (!parsed || last4.length !== 4) {
        return res.status(400).json({
          ok: false,
          error: '受付番号(例: W-123 / Q-45)と、お申し込み時の電話番号の下4桁を入力してください。',
        });
      }
      ...
      if (!intake || !PREFIX_TO_SOURCE[parsed.prefix].includes(intake.line_user_id)) {
