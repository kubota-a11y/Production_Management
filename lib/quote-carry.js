...
// AIが読む期間(AI_REPLY_CONTEXT_DAYS・既定7日)と揃える。前回の注文の見積を今回の登録に入れないため
...
function sanitizeHint(hint) {
  ...
  return {
    ...
    // 売上区分(freeeの勘定科目の振り分け先・2026-09-24)。見積で決めた区分を案件へ運ぶ
    sales_category: salesCategory.normalize(hint.sales_category) || null,
  };
}
...
function hintFromConditions(cond) {
  ...
  return sanitizeHint({
    ...
    // AIの見積条件からも売上区分を仮置きする(KRATVS→カスタム・八木繊維→企業ユニ・昇華→昇華)
    sales_category: salesCategory.suggest({
      mode: cond.mode, customer: cond.customer_name, title: cond.title,
      texts: (cond.rows || []).map((r) => r.method === 'sublimation' ? '昇華' : ''),
    }).code,
  });
}
