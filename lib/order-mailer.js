...
async function sendOrderConfirmation({ to, receiptNo, requestTypeLabel, orderer, summary }) {
  ...
  // 進捗確認ページの案内(公開URLが設定されている環境でのみ出す)
  if (process.env.PUBLIC_ORDER_BASE_URL) {
    L.push('');
    L.push('■進捗の確認');
    L.push(`${process.env.PUBLIC_ORDER_BASE_URL.replace(/\/$/, '')}/status`);
    L.push('上記ページで、受付番号とお申し込み時のお電話番号の下4桁をご入力いただくと、');
    L.push('いつでも進行状況をご確認いただけます。');
  }
  L.push('');
