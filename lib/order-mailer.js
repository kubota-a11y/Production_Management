...
// lineFollowup({ url }) を渡すと「LINEでのやり取りをご希望の方」の案内を足す(公式LINE入口フォーム用。
// 受付番号入りの本文が入力済みでトークが開くリンク。お客様は送信を押すだけでチャットが始まる)。
async function sendOrderConfirmation({ to, receiptNo, requestTypeLabel, orderer, summary, progress, lineFollowup }) {
  ...
  // ユーザー名は取引先の加工依頼のみ(着用される会社・団体名)。
  // 取引先が「どの受付番号がどのユーザー向けか」を控えメールだけで照合できるようにする
  if (summary.user_name) L.push(`■ユーザー名: ${summary.user_name}`);
  ...
  if (lineFollowup && lineFollowup.url) {
    L.push('');
    L.push('■LINEでのやり取りをご希望の方');
    L.push('下のリンクを開くと、受付番号入りのメッセージが用意された状態で公式LINEのトークが開きます。');
    L.push('そのまま送信していただくと、担当がそのトークでご連絡します。');
    L.push(lineFollowup.url);
  }
  ...
  // 件名にもユーザー名を入れる(受信箱の一覧で受付番号と突き合わせられるようにするため)
  const subjectUser = summary.user_name ? ` / ユーザー: ${summary.user_name}` : '';
  await getTransporter().sendMail({
    from: process.env.MAIL_FROM,
    to,
    subject: `【受付完了】${requestTypeLabel}を受け付けました(受付番号 ${receiptNo}${subjectUser})`,
    text: L.join('\n'),
  });
  return true;
}
...
async function sendOrderNotificationToAdmin({ receiptNo, requestTypeLabel, orderer, summary, progress }) {
  ...
  await getAdminTransporter().sendMail({
    ...
    subject: `【新規受付 ${receiptNo}】${orderer.org_name || orderer.contact_name} 様より${requestTypeLabel}`
      + (summary.user_name ? `(ユーザー: ${summary.user_name})` : ''),
    text: L.join('\n'),
  });
  return true;
}
