...
// メールアドレスとユーザー名の必須チェック(新規・追加の両方で共通)。
// 2026-09-16 までは任意だったが、控えメールを本人に届けること・受付番号とユーザーの
// 突き合わせを確実にするため必須にした(八木繊維 木之下さん要望)
function validateRequiredContact(email, userNameRaw, push) {
  if (!isNonEmptyStr(email)) push('dropoff.email', 'メールアドレスを入力してください');
  else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) push('dropoff.email', 'メールアドレスの形式が不正です');
  if (!isNonEmptyStr(s(userNameRaw))) push('user_name', 'ユーザー名(着用される会社・団体名)を入力してください');
}
...
function validateAdditional(body, fileCount) {
  ...
  // メール・ユーザー名は 2026-09-16 に必須化(八木繊維 木之下さん要望)。
  // 控えメールを本人へ届け、受付番号とユーザー(着用される会社・団体)を突き合わせられるようにする
  validateRequiredContact(email, body.user_name, push);
