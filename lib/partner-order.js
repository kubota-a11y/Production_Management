...
const { sendOrderNotificationToAdmin } = require('./order-mailer');
...
function validateAdditional(body, fileCount) {
  ...
  return {
    ...
    dropoff: { date: null, instruction_no, contact_name, phone },
    items: [],
    ...
    try {
      ...
      try {
        const sent = await sendOrderNotificationToAdmin({
          ...
          orderer: { org_name: link.partner_name, contact_name: data.dropoff.contact_name, phone: data.dropoff.phone || '(未記入)', email: '' },
          summary: { items: itemsColumn(data), quantity: quantityColumn(data), deadline: deadlineColumn(data), contact_time: null },
        });
