...
function calcProcessing(input) {
  ...
  // 割増は「元の単価 × (1 + 各割増の増分の合計)」(重ね掛けしない・2026-09-25 社長指示)。/quote-sim と同じ式
  const surAdd = surcharges.filter((k) => w.QS_SURCHARGE[k] && !(tbl.bringFree && k === 'bring')).reduce((s, k) => s + (surRate(k, mode) - 1), 0);
  const minFee = tbl.minFeeApplies && minQty < 10 ? w.QS_COMMON.minFeeRate : 1;
  const exprAdd = express ? 0.5 : 0;
  const mul = (u) => up10(u * (1 + surAdd + (minFee - 1) + exprAdd));
  const mulNoMin = (u) => up10(u * (1 + surAdd + exprAdd));
  const notes = [];
...
function lookupBody(input) {
  ...
  const bodies = hits.slice(0, 8).map((b) => {
    ...
    // 色区分(ホワイト/カラー/アッシュ)の違いで単価が変わるため、区分名を頭に付けて返す
    const bands = sizes ? sizes.v.flatMap((v) => v.b.map(([label, price]) => `${v.l ? `${v.l} ` : ''}${label}:${price}円`)).slice(0, 16) : [];
    return {
