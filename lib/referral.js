...
const { notifyReferralIssued, syncReferralPartners } = require('./todo-notify');
...
function shareUrlOf(code) {
  const base = publicOrderBase();
  return base ? `${base}/order?ref=${encodeURIComponent(code)}` : '';
}
...
function registerReferralRoutes(app, db) {
  ...
  app.post('/api/referral/verify', (req, res) => {
    try {
      ...
      return res.json({
        ...
        share_url: shareUrlOf(partner.code),
        stats,
      });
    } catch (err) {
  ...
  app.post('/api/referral-partners', (req, res) => {
    try {
      ...
      // スプレッドシートの紹介コード台帳へ1行追記する。
      // ファイア&フォーゲットなので、シートが落ちていても発行はここまでで成立している
      notifyReferralIssued(
        { code, partner_type: partnerType, partner_name: partnerName, memo, created_at: now },
        shareUrlOf(code)
      );
  ...
  // ---- 社内API: 発行済みコードを台帳へ一括送信 ----
  // 連携を入れる前に発行したコードの棚卸し用。既存行はシート側でスキップされる
  app.post('/api/referral-partners/sync-sheet', async (req, res) => {
    try {
      const rows = db.prepare('SELECT * FROM referral_partners ORDER BY id').all();
      const result = await syncReferralPartners(
        rows.map(r => ({ ...r, share_url: shareUrlOf(r.code) }))
      );
      res.json(result);
    } catch (err) {
      console.error('[紹介] 台帳への一括送信に失敗:', err.message);
      res.status(500).json({ ok: false, error: '台帳へ送信できませんでした' });
    }
  });
