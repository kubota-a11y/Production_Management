...
function validateAndNormalize(payload, fileRoles) {
  ...
  // メールは見積では必須(従来どおり)、正式発注では任意。入力があれば種別を問わず形式チェック。
  if (!isNonEmptyStr(orderer.email)) {
    if (request_type !== 'order') push('orderer.email', 'メールアドレスを入力してください');
  } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(orderer.email)) {
  ...
  if (usProvided) {
    ...
    // 第2カテゴリは任意。選択中カテゴリに属さない値は黙って落とす(任意項目のため)。
    let sub_category = s(us.sub_category);
    if (sub_category && !isValidSubCategory(category, sub_category)) sub_category = '';
    // 正式発注ではご予算感/用途/希望の雰囲気は使わない → 万一送られてきても保存しない。
    const estimateOnly = request_type !== 'order';
    unknown_spec = {
      category,
      sub_category,
  ...
  // 送信可の条件(緩和): 大カテゴリが選択されていれば満たす(第2カテゴリ/用途等は不問)。
