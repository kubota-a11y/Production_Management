...
const { findPartnerByCode } = require('./referral');
...
// .heic/.webp対応(2026-07-27): iPhone写真(HEIC)がファイルApp経由で弾かれていた。保存のみで変換不要
const ALLOWED_EXT = new Set(['.jpg', '.jpeg', '.png', '.pdf', '.ai', '.svg', '.heic', '.webp']);
...
function validateItem(raw, request_type, prefix, label) {
  ...
  if (q.matrix && typeof q.matrix === 'object' && Array.isArray(q.matrix.cells)) {
    ...
    // セル数と1セルの数量に上限を設ける(以前は上限なしで、1セル9億枚でも通っていた)
    if (q.matrix.cells.length > 200) {
      push('quantity.matrix', '数量の内訳が多すぎます(上限200行)');
    }
    for (const c of q.matrix.cells) {
      ...
      if (Number.isInteger(qty) && qty > 9999) {
        push('quantity.matrix', '1件あたりの数量は9999枚までで入力してください(大口のご注文はお電話にてご相談ください)');
        break;
      }
    }
...
function validateAndNormalize(payload, fileRoles) {
  ...
  // 紹介コード(任意)。紹介カード/紹介ページ(?ref=)経由の申告。
  // コード形式(HY-〇〇)以外に「◯◯さんの紹介」のような自由記述も受け付ける(入力で注文を止めない)
  const referral_code = s(p.referral_code, 50);
  ...
  if (request_type === 'consult') {
    ...
    data = { request_type, orderer, items, roster, deadline, remarks, referral_code, schema_version: 2 };
...
function itemDetailLines(it, ind, qtyLabel) {
  ...
  if (it.item_spec.unknown_spec) {
    ...
    // 品番も一緒に入っているときは「品番不明」だと誤読されるのでラベルを変える
    // (サイトのシミュレーターからの引き継ぎは品番+カテゴリの両方が入ってくる)
    L.push(`${ind}${it.item_spec.catalog_items.length ? 'ご希望の内容' : '品番不明'}: ${seg.join(' / ')}`);
  }
...
function buildNotes(data, savedImages, saveDir, receiptNo) {
  ...
  if (data.request_type === 'consult') {
    ...
    if (u) {
      ...
      if (u.budget) seg.push(`予算:${u.budget}`);
      if (u.mood) seg.push(`雰囲気:${u.mood}`);
      itemLines.push(`  ①${data.item_spec.catalog_items.length ? 'ご希望の内容' : '品番不明'}: ${seg.join(' / ')}`);
    }
  ...
  if (isNonEmptyStr(data.referral_code)) {
    // 発行済みのコードなら「誰の紹介か」を名前で出す(未登録のコードはそのまま表示して気づけるようにする)
    const p = data._referralPartner;
    L.push(p
      ? `■紹介: ${p.partner_name}(${p.type_label}) からのご紹介 / コード ${data.referral_code}`
      : `■紹介コード: ${data.referral_code} ※未登録のコードです。内容をご確認ください`);
  }
...
function registerOrderRoutes(app, db) {
  ...
  app.post('/order', uploadMiddleware, async (req, res) => {
    ...
    try {
      ...
      // 紹介コードが発行済みのものなら、誰の紹介かをメモに出せるよう引いておく
      if (data.referral_code) {
        data._referralPartner = findPartnerByCode(db, data.referral_code);
      }
      ...
      const insert = db.prepare(`
        INSERT INTO ai_extracted_intake
          (line_user_id, extracted_at, customer_name, items, quantity, deadline, notes, raw_ai_response, message_ids, referral_code)
        VALUES ('WEB', ?, ?, ?, ?, ?, ?, ?, '[]', ?)
      `);
      ...
      // 7以降で失敗した場合はINSERT済みレコードを取り消す。
      // 残したままだと、お客様には500(エラー)が見えているのに受付だけ成立しており、
      // 再送信されると二重受付になるため
      try {
      ...
      notifyIntakeTask(`Web注文フォーム対応: ${data.orderer.org_name || data.orderer.contact_name} — ${itemsColumn(data) || 'かんたん相談'}(受付 ${receiptNo})`);
      ...
      return res.status(201).json({ ok: true, intake_id: intakeId, receipt_no: receiptNo, receipt_mail: willSendMail, request_type: data.request_type, ...(warning ? { image_warning: true } : {}) });
        ...
        try {
          ...
          console.error(`[注文] 受付処理が途中で失敗したため intake_id=${intakeId} を取り消しました`);
        } catch (delErr) {
          console.error(`[注文] 失敗した受付レコード(intake_id=${intakeId})の取り消しにも失敗:`, delErr.message);
        }
        throw postInsertErr;
      }
    } catch (err) {
...
module.exports = {
  registerOrderRoutes, validateAndNormalize, MIN_LEAD_DAYS, WEB_ORDER_RECEIVED_PATH,
  // 公式LINE入口フォーム(lib/inquiry.js)が同じ画像保存先・レート制限・bot対策を使うために公開(2026-09-09)
