...
const REQUEST_TYPES = new Set(['quote', 'order']);
const CATEGORIES = new Set(['tshirt', 'polo', 'workwear', 'other']);
...
const CATEGORY_LABEL = { tshirt: 'Tシャツ', polo: 'ポロシャツ', workwear: '作業着', other: 'その他' };
const REQUEST_TYPE_LABEL = { quote: '見積・イメージ依頼', order: '正式発注' };
...
function validateAndNormalize(payload, fileRoles) {
  ...
  if (!isNonEmptyStr(orderer.org_name)) push('orderer.org_name', '会社/団体名を入力してください');
  ...
  if (!isNonEmptyStr(orderer.email)) push('orderer.email', 'メールアドレスを入力してください');
  else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(orderer.email)) push('orderer.email', 'メールアドレスの形式が不正です');

  // アイテム指定(①②③のいずれか必須・併用可)
  const is = (p.item_spec && typeof p.item_spec === 'object') ? p.item_spec : {};
  ...
  const usProvided = us && typeof us === 'object'
    && (isNonEmptyStr(us.category) || isNonEmptyStr(us.purpose) || isNonEmptyStr(us.budget) || isNonEmptyStr(us.mood));
  if (usProvided) {
    const category = s(us.category);
    if (!CATEGORIES.has(category)) push('item_spec.unknown_spec.category', 'カテゴリを選択してください');
    unknown_spec = {
      category,
      purpose: s(us.purpose, LEN.mid),
      budget: s(us.budget),
      mood: s(us.mood, LEN.mid),
    };
  }
  const has2 = !!unknown_spec && CATEGORIES.has(unknown_spec.category)
    && (isNonEmptyStr(unknown_spec.purpose) || isNonEmptyStr(unknown_spec.budget) || isNonEmptyStr(unknown_spec.mood));

  const has3 = (fileRoles || []).some(r => r === 'reference');
  ...
  // 加工内容
  const d = (p.decoration && typeof p.decoration === 'object') ? p.decoration : {};
  ...
  // 数量
  const q = (p.quantity && typeof p.quantity === 'object') ? p.quantity : {};
  ...
  // A方針: 正式発注は「マトリクス合計≥1 or 概数の自由記述」のどちらか必須(完全な空は不可)。見積は任意。
  if (request_type === 'order' && matrixTotal < 1 && !isNonEmptyStr(approximate)) {
...
function buildNotes(data, savedImages, saveDir) {
  ...
  if (m && m.total >= 1) {
    const brk = m.cells.slice(0, 12)
      .map(c => `${c.size || ''}${c.color || ''}${c.qty}`).join(', ');
    ...
    L.push(`■数量(概数): ${data.quantity.approximate}`);
  }
...
function registerOrderRoutes(app, db) {
  ...
  app.post('/order', uploadMiddleware, async (req, res) => {
    ...
    try {
      ...
      const finalNotes = buildNotes(data, images, dir);
      ...
      return res.status(201).json({ ok: true, intake_id: intakeId, request_type: data.request_type, ...(warning ? { image_warning: true } : {}) });
    } catch (err) {
