  ...
  switch (toStage) {
    ...
    case 'REVIEW':
      ...
      return fromStage === 'DESIGN'
        ? `✅ ${carveMark}【${label}】初校が提出されました → 三浦さん又は山本さん: お客様へ確認連絡`
        : null;
    case 'PRODUCTION':
    ...
    case 'BILLING':
      return isPaper
        ? `💰 ${carveMark}【${label}】入稿が完了しました → 三浦さん又は山本さん: 請求書を発行して完了に`
        : `💰【${label}】請求の段階です → 三浦さん又は山本さん: 請求書の発行`;
    case 'INSPECTION':
      return `🔍【${label}】検品をお願いします → 三浦さん・渡邉さん`;
    case 'DELIVERY':
      return `🚚【${label}】納品の段階です → 三浦さん又は山本さん: お渡し・発送`;
    case 'DONE':
